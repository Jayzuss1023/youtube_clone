...
Django renders HTML templates. Uploads go to ImageKit; the database only stores metadata (`file_id`, `video_url`, thumbnail URL, counts). Playback uses ImageKit's HLS URL when possible, plus a quality transform. If the user didn't upload a thumbnail, ImageKit can generate one from the video.
...
```
Browser  →  Django (templates + views)  →  SQLite locally / Postgres on Vercel
                 ↓
              ImageKit (video files, thumbnails, streaming URLs)
```
...
- [uv](https://docs.astral.sh/uv/) for the project/env
...
├── pyproject.toml
├── README.md
└── youtube/                 # Django project root (run manage.py from here)
    ├── manage.py
    ├── templates/           # shared base layout
    ├── static/css/          # navbar, player, forms, etc.
    ├── accounts/            # register / login / logout
    ├── videos/              # feed, upload, detail, channel, votes
    │   ├── models.py        # Video, VideoLike
    │   ├── views.py
    │   ├── forms.py
    │   ├── imagekit_client.py
    │   └── templates/videos/
    └── youtube/             # settings, root URLs
```
